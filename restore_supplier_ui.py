...
new_content = """// Supplier Portal — All Pages
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <button className="btn-primary btn-sm"><Plus className="w-4 h-4" /> Add Tanker</button>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: 'Total', value: myTankers.length, color: 'text-dark' },
          { label: 'Available', value: myTankers.filter(t => t.status === 'available').length, color: 'text-emerald-600' },
          { label: 'In Use', value: myTankers.filter(t => t.status === 'in_use').length, color: 'text-primary-600' },
        ].map(s => (
      ...
      <div className="card">
        <div className="table-container">
          <table className="table">
            ...
            <tbody>
              {myTankers.slice(0, 15).map(t => {
                const driver = myDrivers.find(d => d.tankerIds.includes(t.id));
                return (
                  <tr key={t.id}>
                    <td><span className="font-mono text-xs">{t.registrationNumber}</span></td>
                    ...
                    <td>
                      <div className="flex items-center gap-2">
                        <div className="flex-1 bg-slate-100 rounded-full h-1.5 w-16">
                          <div className="h-1.5 bg-primary-500 rounded-full" style={{ width: `${(t.currentLoad/t.capacity)*100}%` }} />
                        </div>
                        <span className="text-xs text-muted">{Math.round(t.currentLoad/t.capacity*100)}%</span>
                      </div>
                    </td>
                    <td>
